...
float DrumEngine::render(float color) {
    ...
    // 🔥 KICK NUEVO (simple, sólido, sin cosas raras)
    if (kick_env_ > 0.0005f) {
        float env = kick_env_;

        // pitch drop MUCHO más controlado (no sweep loco)
        float freq = 50.0f + 70.0f * env; // rango corto y musical
    ...
    // SNARE (igual que antes, funciona)
