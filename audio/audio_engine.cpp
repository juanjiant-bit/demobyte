...
void AudioEngine::process_one_sample() {
    ...
    if (note_voice_.active) {
        ...
        const float fm_shaped = voice_shaper(
            fm_voice,
            0.22f + note_fold_mod,
            0.18f + 0.10f * note_fm_mix_ + 0.06f * mod_bus_.note_decay
        );
    ...
    const float breath_bass_scale =
        0.88f + 0.28f * mod_bus_.bass_gate + 0.16f * mod_bus_.bass_decay;
