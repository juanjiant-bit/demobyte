#pragma once
// state_manager.h — BYT3/YUY0 V1.22
//
// BUG-SM-1 CORREGIDO: #include <stdint.h> estaba ANTES de #pragma once.
// Esto causaba que el include guard no funcionara si el header se incluía
// desde múltiples translation units antes de que el compilador procesara
// el pragma. Movido #include después del pragma once.
//
// BUG-SM-2 CORREGIDO: set_graph_param_on_live_graphs() faltaba
// begin_live_write() al inicio — solo tenía end_live_write() al final.
// Esto significaba que Core0 podía leer un estado de graphs[] parcialmente
// actualizado sin el fence de memoria correcto al inicio de la escritura.
...
public:
    ...
    void    fill_context(EvalContext& ctx) const;
    ...
    void set_drum_color_live(float v) {
        ...
        begin_live_write(); drum_color_live_ = v; end_live_write();
    }
    void set_drum_decay_live(float v) {
        if (v < 0.0f) v = 0.0f; if (v > 1.0f) v = 1.0f;
        begin_live_write(); drum_decay_live_ = v; end_live_write();
    }
    void set_duck_amount_live(float v) {
        if (v < 0.0f) v = 0.0f; if (v > 1.0f) v = 1.0f;
        begin_live_write(); duck_amount_live_ = v; end_live_write();
    }
    void set_reverb_wet_live(float v) {
        if (v < 0.0f) v = 0.0f; if (v > 1.0f) v = 1.0f;
        begin_live_write(); reverb_wet_live_ = v; end_live_write();
    }
    ...
    void set_note_voice_source(uint8_t v) {
        begin_live_write(); note_voice_source_ = (v > 2u) ? 0u : v; end_live_write();
    }
    uint8_t note_voice_source() const { return note_voice_source_; }
    void cycle_note_voice_source() {
        begin_live_write();
        note_voice_source_ = (uint8_t)((note_voice_source_ + 1u) % 3u);
        end_live_write();
    }
    ...
    void set_active_note(uint8_t degree, uint8_t midi) {
        begin_live_write();
        note_active_ = true; note_degree_ = degree; note_midi_ = midi;
        end_live_write();
    }
    void clear_active_note() {
        begin_live_write();
        note_active_ = false; note_degree_ = 0xFFu; note_midi_ = 0xFFu;
        end_live_write();
    }
    ...
    void set_mute_snap(uint8_t slot, bool v) { if (slot < NUM_SNAPSHOTS) mute_snap_[slot] = v; }
...
private:
    ...
    uint8_t       scale_id_live_    = 1;
    uint8_t       root_live_        = 0;
    float         drum_color_live_  = 0.0f;
    float         drum_decay_live_  = 0.5f;
    float         duck_amount_live_ = 0.0f;

    volatile bool     note_mode_active_    = false;
    volatile float    note_pitch_ratio_    = 1.0f;
    volatile bool     note_active_         = false;
