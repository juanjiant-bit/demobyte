...
void AdcHandler::select_channel(uint8_t ch) {
    gpio_put(MUX_S0, ch & 0x01u);
    gpio_put(MUX_S1, (ch >> 1) & 0x01u);
    gpio_put(MUX_S2, (ch >> 2) & 0x01u);
}

uint16_t AdcHandler::read_adc() {
    return adc_read();
}
...
void AdcHandler::poll() {
    auto read_mux_avg = [this](uint8_t ch) -> uint16_t {
        ...
        (void)read_adc();
        uint32_t acc = 0;
        for (uint8_t n = 0; n < OVERSAMPLE; ++n) {
            acc += read_adc();
        }
    ...
    for (uint8_t i = 0; i < NUM_POTS - 1; ++i) {
        ...
        if (delta < (int32_t)HYST) {
            raw = last_raw_[i];
        } else {
            last_raw_[i] = raw;
        }
        smoothed_[i] += SMOOTH * ((raw / 4095.0f) - smoothed_[i]);
    }
...
float AdcHandler::get(uint8_t idx) const {
